...
def get_openapi_schema_from_marshmallow_field(
    ...
    if isinstance(marshmallow_field, marshmallow.fields.Nested):
        if isinstance(marshmallow_field.nested, str):
            return {
                "$ref": f"#/components/schemas/{strip_schema_from_name(marshmallow_field.nested)}"
            }
        else:
            return {
                "$ref": f"#/components/schemas/{strip_schema_from_name(marshmallow_field.nested.__name__)}"
            }
    ...
    if issubclass(data_type, marshmallow.Schema):
        openapi_schema = {
            class_name: {
                ...
                "type": "object",
                "properties": {
                    name: get_openapi_schema_from_marshmallow_field(field)
                    for name, field in data_type._declared_fields.items()
                },
            }
        }
        for _, field in data_type._declared_fields.items():
            if isinstance(field, marshmallow.fields.Nested):
                ...
                if (
                    strip_schema_from_name(nested_schema.__name__)
                    not in openapi_schema.keys()
                ):
                    openapi_schema.update(
                        get_openapi_schema_from_mashmallow_schema(
